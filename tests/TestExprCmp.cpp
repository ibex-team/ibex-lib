...
void TestExprCmp::test01() {
	...
	TEST_ASSERT(x1==x1);
	TEST_ASSERT((x1+1)==(x1+1));
	TEST_ASSERT((x1+x2)==(x1+x2));
	TEST_ASSERT((exp(x1+x2)-1)==(exp(x1+x2)-1));
	TEST_ASSERT((x1+x2*x1)==(x1+x2*x1));
...
void TestExprCmp::test02() {
	...
	TEST_ASSERT(x1!=x2);
	TEST_ASSERT(x1!=ExprConstant::new_scalar(Interval::ZERO));
	TEST_ASSERT((x1+1)!=(x1-1));
	TEST_ASSERT((x1+x2)!=(x1-x2));
	TEST_ASSERT((x1+x2*x1)!=(x1+x1*x2));
...
void TestExprCmp::test03() {
	...
	Function f(x,x+1);
	TEST_ASSERT(f(x1)==f(x1));
	TEST_ASSERT(f(x2)!=f(x1));
}
