...
// The next function is necessary because we need
// an extended system to build arguments of the base
// class constructor (ctc, bsc, loup finder, etc.)
...
#define EXT_SYS get_ext_sys(sys,eps_h)
...
DefaultOptimizer::DefaultOptimizer(const System& sys, double eps_x, double rel_eps_f, double abs_eps_f, double eps_h, bool rigor, double random_seed) :
		Optimizer(sys.nb_var,
			  ctc(EXT_SYS), // warning: we don't know which argument is evaluated first
			  rec(new SmearSumRelative(EXT_SYS,eps_x)),
			  rec(rigor? (LoupFinder*) new LoupFinderCertify(EXT_SYS,rec(new LoupFinderDefault(EXT_SYS))) :
						 (LoupFinder*) new LoupFinderDefault(EXT_SYS)),
			  (CellBufferOptim&) rec(new CellDoubleHeap(EXT_SYS)),
			  EXT_SYS.goal_var(),
			  eps_x,
			  rel_eps_f,
			  abs_eps_f) {
