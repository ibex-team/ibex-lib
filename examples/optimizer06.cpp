...
int main(int argc, char** argv){
	...
	RNG::srand(1);
	NormalizedSystem ext_sys(sys,eqeps);
	std::cout<< ext_sys.ctrs[0].f.nb_var()<<"    "<<sys.nb_var<<"    "<<ext_sys.nb_var<<std::endl;
	vector<Ctc*> array_in;
	for (int i=0;i<ext_sys.nb_ctr;i++)
		array_in.push_back(new CtcFwdBwd(ext_sys.ctrs[i].f,GT));

	CtcUnion ctc_in(array_in);
	...
	OptimCtc oo(*ctcxn,ctc_in,*sys.goal,*bs,prec,goalprec,goalprec);
