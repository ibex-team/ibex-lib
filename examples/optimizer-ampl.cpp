...
int main(int argc, char** argv){
	...
	if (argc<9) {
		cerr << "usage: optimizer04 filename filtering linear_relaxation bisection prec goal_prec timelimit randomseed "  << endl;
		exit(1);
	}
	...
	int randseed= atoi(argv[8]);
	double taumohc=0.8;
	srand(randseed);
	double eq_eps= 1.e-8;
	double ratio=0.5;
	// the extended system 
	ExtendedSystem ext_sys(sys,eq_eps);
	...
	if (bisection=="roundrobin")
	  bs = new RoundRobin (prec,ratio);
	else if (bisection== "largestfirst")
          bs= new LargestFirst(prec,ratio);
	else if (bisection=="smearsum")
	  bs = new SmearSum(ext_sys,prec,ratio);
	else if (bisection=="smearmax")
	  bs = new SmearMax(ext_sys,prec,ratio);
	else if (bisection=="smearsumrel")
	  bs = new SmearSumRelative(ext_sys,prec,ratio);
	else if (bisection=="smearmaxrel")
	  bs = new SmearMaxRelative(ext_sys,prec,ratio);
	/*
	...
	*/
	...
	CtcMohc mohc(ext_sys.ctrs,0.01,false, 0.01, CtcMohc::default_univ_newton_min_width, taumohc);
	CtcMohc mohc4cid(ext_sys.ctrs,mohc.active_mono_proc,0.1,true, 0.1, CtcMohc::default_univ_newton_min_width);
	...
	Ctc3BCid c3bcidobjhc4(ext_sys.nb_var,hc44cid,10,1,1);
	...
	CtcAcid acidmohc(ext_sys,mohc4cid,true);
	...
	Optimizer o(sys,*ctcxn,*bs,prec,goalprec,goalprec,samplesize,eq_eps,false,100,CellHeapOptim::UB);
	//	Optimizer o(sys,*ctcxn,*bs,prec,goalprec,goalprec,samplesize,eq_eps,false,50,CellHeapOptim::PU);
	...
	if (o.trace)
	 o.report();
	  ...
	  {cout << o.time << " " << o.nb_cells << " "
	      // << acidhc4.nbvar_stat() 
		<<  endl;}
	
	// o.time_cells_report();

	if (o.trace && (filtering == "acidhc4" || filtering == "acidmohc" ))
