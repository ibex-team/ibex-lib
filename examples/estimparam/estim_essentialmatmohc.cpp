	...
	*/
	...
	for (int i=0; i<p; i++) {
	  ...
	  */
	    ...
	    m_func[i]= new Function (w,(w[0][0]* (x1.at(i) * x2.at(i) * a1*a1  + x1.at(i) * a1 * a3 + x2.at(i) *a1* a3 + a3*a3 ) 
				      +  w[0][1] * (x1.at(i)*y2.at(i)*a1*a5 + x1.at(i)* a1* a6 + y2.at(i)*a3*a5 + a3*a6)
				      +  w[0][2]*(x1.at(i) * a1 + a3) 
				      +  w[1][0]*(x2.at(i) *  y1.at(i) * a1*a5 + x2.at(i)* a1*a6 + y1.at(i) * a3*a5 + a6*a3)
				      +  w[1][1] * ( y1.at(i) * y2.at(i) * a5*a5 +  y1.at(i) * a5 * a6 + y2.at(i)*a5 * a6 + a6*a6)
				      +  w[1][2] * ( y1.at(i)* a5 + a6) 
				      +  w[2][0]* (x2.at(i) *a1+ a3) 
				      +  w[2][1] * ( y2.at(i)* a5 + a6) 
				      +  w[2][2]+Interval(-epseq,epseq)));
...
// 
	...
	*/
	...
	CtcHC4 hc44cid(sys0,0.1,true);
	double taumohc=0.8;
	CtcMohc mohc(sys0.ctrs,0.01,false, 0.01, CtcMohc::default_univ_newton_min_width, taumohc);
	CtcMohc mohc4cid(sys0.ctrs,mohc.active_mono_proc,0.1,true, 0.1, CtcMohc::default_univ_newton_min_width);
	CtcAcid acidmohc(sys0,mohc4cid,true);
	...
	//      	CtcCompo ctcqf0(mohc,  acidmohc,lphull,ctcq);
	//	CtcCompo ctcqf0(hc44cid, acidhc4,lphull,ctcq);
	//	CtcCompo ctcqf0(hc44cid, lphull,ctcq);
	//	CtcCompo ctcqf0(mohcacidmohc, lphull,ctcq);
	CtcCompo ctcqf0(mohc, lphull,ctcq);
