...
x->push_back(300- y->at(nn*i) - z-> at(nn*i)+ (((double)rand()/(double)RAND_MAX)*0.0001));
	    x->push_back(- y->at(nn*i+1)  + z-> at(nn*i+1)+ (((double)rand()/(double)RAND_MAX)*0.0001));	
	    x->push_back(-200+ y->at(nn*i+2)  + z-> at(nn*i+2)+ (((double)rand()/(double)RAND_MAX)*0.0001));
	    x->push_back( y->at(nn*i+3)  - z-> at(nn*i+3)+ (((double)rand()/(double)RAND_MAX)*0.0001));	

	    x->push_back(320- y->at(nn*i+4) - z-> at(nn*i+4)+ (((double)rand()/(double)RAND_MAX)*0.0001));
	    x->push_back(10 - y->at(nn*i+5)  +z-> at(nn*i+5)+ (((double)rand()/(double)RAND_MAX)*0.0001));
	    x->push_back(5- y->at(nn*i+6)  + z-> at(nn*i+6)+ (((double)rand()/(double)RAND_MAX)*0.0001));
	    x->push_back(15- y->at(nn*i+7)  +z-> at(nn*i+7)+ (((double)rand()/(double)RAND_MAX)*0.0001));
	...
	for (int oct=0; oct <4; oct++)
	  ...
	  //					 *new Ctc3BCid(3, * new CtcFwdBwd(*m_func[i]),5,1,3)));
	    ...
	    */
	    ...
	    //	    CtcQInterPlane ctcq(n,p,m_ctc1,linfun,epseq,Q,QINTERCORE,K);
	    //	    CtcQInterPlane  ctcq(n,p,m_ctc1,linfun,epseq,Q, QINTERCORE,K );
	    //	    CtcQInterPlane ctcq(n,p,m_ctc1,linfun,epseq,Q,QINTERFULL, K);
