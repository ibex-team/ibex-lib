/* Fundamental matrix estimation   with v[8]=1   */
...
int main(int argc, char** argv) {
	...
	*/
	m_norm = new Function (v, sqrt (sqr(v[0])+sqr(v[1])+sqr(v[2])+ sqr(v[3]) + sqr(v[4]) + sqr(v[5]) + sqr(v[6]) + sqr(v[7]) +1));


	m_det = new Function (v, v[0]*(v[4]-v[7]*v[5]) - v[3]*(v[1] - v[2]*v[7] ) + v[6]*(v[1]*v[5]-v[2]*v[4]) + eps2);
	m_det1 = new Function (v, v[0]*(v[4]-v[7]*v[5]) - v[3]*(v[1] - v[2]*v[7] ) + v[6]*(v[1]*v[5]-v[2]*v[4]));
	...
	*/
	...
	// meilleure solution à ransac 153   tolerance 0.001
	/*	
	fundmat[0]=0.00340751;
	fundmat[3]=0.263392;
        fundmat[6]=-1.48283;
	fundmat[1]=  -0.257501;
	fundmat[4]=0.0128679 ;
	...
	*/
	...
	//	CtcQInterAffLinear ctcq(8,m_ctc,linfun,epseq,Q,QINTERCORE);
	//	CtcQInterAffLinear ctcq(8,m_ctc,linfun,epseq,Q,QINTERFULL);
	CtcQInterAffLinearv8 ctcq(8,m_ctc,linfun,epseq,Q);
	...
	//	OptimQInter s(ctcqf0,bs,buff,ctcq);
	//	SolverQInter s(ctcqf0,bs,buff,ctcq,1);
	SolverOptConstrainedQInter s(sys1,ctcq,bs,buff,ctcq,1);
	//	OptimizerQInter s(ctcqf0,bs,buff,ctcq,1);
	...
	cout << "det best sol  " <<  m_det->eval(bestsolbox) << endl;
	cout << "norm best sol " << m_norm->eval(bestsolbox) << endl;
        int nbinliers=0;
