...
const double Qprop = 0.0151;							//Minimum ratio of consistent measurements
...
int main() {
	...
	for (int i=0;i<100;i++)
	  {x->push_back(10+ (((double)rand()/(double)RAND_MAX)*0.001));
	    ...
	    //	    z->push_back(100 + (((double)rand()/(double)RAND_MAX)*0.001));
	    x->push_back(100 + (((double)rand()/(double)RAND_MAX)*0.001));
	    //	    z->push_back(160 + (((double)rand()/(double)RAND_MAX)*0.001));
	    //	    z->push_back(200 + (((double)rand()/(double)RAND_MAX)*0.001));
	    x->push_back(rand()%256);
	    x->push_back(rand()%256);
	    x->push_back(rand()%256);
  	    x->push_back(rand()%256);
	...
	*/
	...
	//	System* systemlr[np];
	//        LinearRelax* lrtaylor[np];
	//	CtcPolytopeHull* polytopeh[np];
	Variable v(3);
	//	double R[p];
	//	double alpha[p];
	clock_t start, end;
	...
	for (int oct=0; oct <4; oct++)
	  ...
	  { 
	    ...
	    m_ct0= new NumConstraint(*m_f0,LEQ);
	    ...
	    */
		...
		for (int nt=0; nt<NT2;nt++)
		  for (int i=0; i<p; i++) {
		    ...
		    fac.add_ctr(*m_func[i]);
		    fac.add_ctr(*m_func[j]);
		    fac.add_ctr(*m_func[k]);
		    fac.add_ctr(*m_ct0);

		    System sys(fac);
		    */
		    //		    m_ctc1.set_ref(p*nt+i,* new CtcCompo (m_ctc[i],m_ctc[j],m_ctc[k]));
		    //		    LinearRelaxXTaylor lrt (sys,cpoints);

		    //		    LinearRelaxCombo* lrt = new  LinearRelaxCombo (sys,LinearRelaxCombo::ART);
		    // LinearRelaxCombo lrt  (sys,LinearRelaxCombo::ART);
		    ...
		    /*
		    if (!(box1->is_empty()))
		      {cout << p*nt+i << " " << x->at(i) << "  " << y->at(i) << "  " << z->at(i)  << endl;
			cout << p*nt+i << " " << x->at(j) << "  " << y->at(j) << "  " << z->at(j)  << endl;
			cout << p*nt+i << " " << x->at(k) << "  " << y->at(k) << "  " << z->at(k)  << endl;
			cout << *box1 << endl;
		      }
		    */
	    ...
	    //	    CtcQInterAff ctcq(3,m_ctc1,Q,flist,QINTERPROJ,K);
	    //	    CtcQInterPlane ctcq(n,m_ctc1,linfun,systemlr,epseq,Q,flist,QINTERPROJ,K);
	    //	     CtcQInterPlane ctcq(n,m_ctc1,linfun,boxes,epseq,Q,flist,QINTERPROJ,K);
	     CtcQInterPlane ctcq(n,m_ctc1,linfun,epseq,Q,flist,QINTERPROJ,K);
	    //	    CtcQInterPlane ctcq(n,m_ctc1,linfun,boxes,epseq,Q,flist,QINTERFULL,K);
	    //	    CtcQInterPlane ctcq(n,m_ctc1,linfun,boxes,epseq,Q,flist,QINTERCORE,K);
	    //	    CtcQInterPlane  ctcq(n,m_ctc1,linfun,epseq,Q,flist, QINTERCORE,K );
	    //CtcQInterPlane ctcq(n,m_ctc1,linfun,epseq,Q,flist, QINTERFULL, K);
		...
		if (!(boxes[i]->is_empty()))
		  {//cout << i << " " << *(boxes[i])  << endl;
		    count=ctcq.points_count(*(boxes[i]));
		    if (count >= Q) 
	...
	if (s.greedy)
	  for (int i=0;i<res.size();i++)
	      for (int j=0;j<p;j++)
		{ IntervalVector box= res[i];
		  Interval evalctc= m_func[j]->eval(box);
		  try {m_ctc[j].contract(box);
		    if (evalctc.contains(0)) active[j]=false;}
	...
	*/
	...
	//	cout << "avant delete2 " << endl;
	
	for (int i=0; i<p; i++)
	  { delete m_func[i];
	       delete &m_ctc[i];
	  }
	//	cout << "avant delete3 " << endl;
	m_ctc.clear();
	m_ctc1.clear();
