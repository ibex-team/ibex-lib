...
/*
 ...
 void CellSet<T>::contract(double loup, int y, bool remove){
  std::set<Cell*>::iterator it0=cset.begin();
  std::set<Cell*>::iterator it1=cset.end();
  int trouve=0;
  ...
  if (trouve) cset.erase(it1,cset.end());
}
...
void CellSet<T>::contract(double loup, int y, bool remove){
  std::set<Cell*>::iterator it0=cset.end();
  it0--;
  while(it0!=cset.begin()) {
    if((*it0)->box[y].lb() > loup){
      if (remove) delete *it0;
      cset.erase(it0--);
    }
...
template <class T2>
unsigned int CellBiCriteria<T2>::size() const {
	return cset.size();
}
