...
class Optimizer {
 public:
   ...
   *   \param g     - the constraints function g(x) ( the constraint is g(x)<=0 )
   *   \param ctc   - the contractor wrt to g(x)<=0
   ...
   */
    Optimizer(Function& f, const Array<Function>& g, Ctc& ctc, Bsc& bsc, double prec=default_prec,
    		double goal_rel_prec=default_goal_rel_prec, double goal_abs_prec=default_goal_abs_prec, int sample_size=default_sample_size);
    ...
    /** Displays on standard output a report of the last call to optimize.
      *
     ...
     *
     * When f is increasing (resp. decreasing) wrt variable x_i, the interval [x_i]
    ...
    /**
     * \brief Quick check that the box is inner
     */
    bool is_inner(const IntervalVector& box);

    /**
     * \brief Reduce box to an inner box using inHC4 algorithm.
     *
     ...
     *                   means "unknown" and a quick check (see is_inner) is performed.
     *
 ...
 private:
    ...
    /** "loup"= lowest upper bound of the minimum */
    double loup;
