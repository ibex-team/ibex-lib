 ...
 */
class ExprIndexCopy : public virtual ExprVisitor {
public:
	 ...
	 */
	const ExprNode& index_copy(const Array<const ExprSymbol>& old_x, const Array<const ExprSymbol>& new_x, const ExprNode& y, int i);
	 ...
	 */
	const ExprNode& index_copy(const Array<const ExprSymbol>& old_x, const Array<const ExprSymbol>& new_x, const ExprNode& y, int i, int j);
...
protected:
	...
	void visit(const ExprSymbol& old_x, const ExprSymbol& new_x);

	// The clones of each node and its components
	class Clones {
	public:
		Clones(const Dim& d);
		~Clones();
	...
	private:
		...
		const ExprNode* e;     // clone of the node itself (lazy calculation)
		const ExprNode** ei;   // clones of the node indexed once (lazy calculation)
		const ExprNode*** eij; // clones the node indexed twice (lazy calculation)
	};

	NodeMap<Clones*> clone;
