...
inline Interval trace(const IntervalMatrix& x) {
	return UnaryOperator<TRACE,IntervalMatrix,Interval>::fwd(x);
}

inline IntervalVector cross(const IntervalVector& x1, const IntervalVector& x2) {
	return BinaryOperator<CROSS_PRODUCT,IntervalVector,IntervalVector,IntervalVector>::fwd(x1,x2);
}
