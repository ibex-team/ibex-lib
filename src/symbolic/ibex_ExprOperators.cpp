...
static Domain eval_binary_domain(const Domain& x1, const Domain& x2) {
	...
	get<Output>(y)=BinaryOperator<Name,Input1,Input2,Output>::eval(get<Input1>(x1),get<Input2>(x2));
	return y;
}
...
BinaryOperatorDef get_binary_op(const char* op_name) {
	...
	//         This part should be generated by install scripts
	// ##################################################################
	//ADD_BINARY_OPERATOR(SINC,Interval,Interval);
	//ADD_BINARY_OPERATOR(TRACE,IntervalMatrix,IntervalVector);
	// ##################################################################
