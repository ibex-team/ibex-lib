...
class Optimizer : public Paver {
 public:
   ...
   *                  form y=f(x), where y is the last variable
   *   \param goal  - the number of the objective constraint y=f(x)  ; 
   *                  for the simplex call, it is assumed that goal=0 (the objective is the first constraint
...
virtual bool random_probing (const System& sys, const Space& space);
void monotonicity_analysis(const Space& space, const Evaluator& goal);
  ...
  */
  ...
  /** Create an optimizer with constraints with:
