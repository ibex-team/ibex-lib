...
try{
  ...
  if(_mohc2 && fog.g[i].lb()<0 && fog.g[i].ub()>0){
     if(ApplyFmax[i]!=NO || ApplyFmin[i]!=NO){
      ...
      if(og_treated){
         LeftNarrow(i);
	     RightNarrow(i);
         if(LB[i].lb()>RB[i].ub()) throw EmptyBoxException();
     }
    }
    return;
  }
  ...
  switch (ctr.op) {
    ...
    RB[i]&=ini;
    if(RB[i].is_empty()) throw EmptyBoxException();
  ...
  switch (ctr.op) {
    ...
    LB[i]&=ini;
    if(LB[i].is_empty()) throw EmptyBoxException();
...
void Function_OG::_eval_leaves(IntervalVector& box, bool minrevise){
   ...
   for(int i=0; i<n;i++){
      if(occ[i].size()==0) {/*_box[first_occ[i]] = box[i];*/ continue; }
      for(int j=0; j<occ[i].size(); j++){
      //for(int occ=first_occ[i];occ<first_occ[i+1];occ++){
		if(minrevise)
...
void Function_OG::_proj_leaves(IntervalVector& box){
   ...
   for(int i=0; i<n;i++){
      ...
      //~ for(int occ=first_occ[i];occ<first_occ[i+1];occ++){
	    ...
	    if(box[i].is_empty()) throw EmptyBoxException();
      }
   }
}
...
Interval Function_OG::revise(IntervalVector& box, bool minrevise){
  ...
  root_label.i()=(minrevise)? Interval(NEG_INFINITY, 0) : Interval(0, POS_INFINITY);
   ...
   hc4r_ev(_f, root_label, _box,ev); //evaluacion se puede obtener a traves de f?
