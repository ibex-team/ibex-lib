...
CtcIn::CtcIn(Function& f, const Domain& y) : Ctc(f.nb_var()), _f(f), _d(_f.expr().dim) {
	assert(_f.expr().dim==y.dim);
	_d = y;
}

CtcIn::CtcIn(Function& f, const Interval& y) : Ctc(f.nb_var()), _f(f), _d(Dim()) {
	assert(_f.expr().dim==_d.dim);
	_d.i() = y;
}

CtcIn::CtcIn(Function& f, const IntervalVector& y) : Ctc(f.nb_var()), _f(f), _d(_f.expr().dim) {
	assert(_f.expr().dim.is_vector());
	_d.v() = y;
}

CtcIn::CtcIn(Function& f, const IntervalMatrix& y) : Ctc(f.nb_var()), _f(f), _d(_f.expr().dim) {
