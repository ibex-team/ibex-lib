...
CtcPolytopeHull::CtcPolytopeHull(Linearizer& lr, int max_iter, int time_out, double eps, Interval limit_diam) :
		...
		mylinearsolver(nb_var, LPSolver::Sense::Minimize, eps, time_out, max_iter),
...
void CtcPolytopeHull::optimizer(IntervalVector& box) {
	...
	for(int ii=0; ii<(2*nb_var); ii++) {  // at most 2*n calls
		...
		{
			inf_bound[i]=1;
			mylinearsolver.set_sense(LPSolver::Sense::Minimize);
			...
			sup_bound[i]=1;
			mylinearsolver.set_sense(LPSolver::Sense::Maximize);
			mylinearsolver.set_var_obj(i, 1.0);
			stat= mylinearsolver.optimize();
			mylinearsolver.set_var_obj(i, 0.0);
