...
template<>
AffineMain<AF_fAF1>& AffineMain<AF_fAF1>::resize(int n){
	if (n>_n) {
		...
		ibex_error("AffineMain<AF_fAF1>::resize: the new size is less than the previous");
	}
	return *this;
}
...
/** \brief Return (-x) */
template<>
AffineMain<AF_fAF1> AffineMain<AF_fAF1>::operator-() const {
	AffineMain<AF_fAF1> res;
	res._n = _n;
...
template<>
AffineMain<AF_fAF1>& AffineMain<AF_fAF1>::sqrA(const Interval& itv) {
