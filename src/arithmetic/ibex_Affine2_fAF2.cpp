...
template<>
AffineMain<AF_fAF2>& AffineMain<AF_fAF2>::resize(int n) {
	if (n>_n) {
		...
		ibex_error("AffineMain<AF_fAF2>::resize: the new size is less than the previous");
	}
	return *this;
}
...
template<>
double AffineMain<AF_fAF2>::center() const{
	return _elt._val[0];
}
...
template<>
AffineMain<AF_fAF2>& AffineMain<AF_fAF2>::saxpy(double alpha, const AffineMain<AF_fAF2>& y, double beta, double ddelta, bool B1, bool B2, bool B3, bool B4) {
