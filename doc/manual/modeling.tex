...
If more than 6 variables are needed, you need to build an intermediate array for collecting the variables.
...
share all the same ordered set of variables. This set is defined via \cf{add_var} once for all.
...
%{\tt x in [-oo,0]} & declares a $10$-sized vector constant $x=(x[1],\ldots,x[10])$, each with domain $(-\infty,0]$ \\
{\tt x in ([-oo,0];[0,+oo])} & declares a $2$-sized vector constant $x$ with $x[1]\in(-\infty,0]$ and $x[2]\in[0,+\infty)$ \\
%{\tt x in [0,1]} & declares $10$ constants $x[1],\ldots,x[10]$, each with domain $[0,1]$ \\
